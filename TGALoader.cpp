...
bool LoadTGA(Texture * texture, char * filename)				// Load a TGA file
{
	...
	else																	// If header matches neither type
	{
		...
		std::cout << "TGA file be type 2 or type 10" << std::endl;
		fclose(fTGA);
		return false;																// Exit function
	}
...
{																			// TGA Loading code nehe.gamedev.net)
	...
	if(texture->imageData == NULL)											// If no space was allocated
	{
		...
		std::cout << "Could not allcoate memory for image" << std::endl;
	...
	{
		texture->imageData[cswap]   = texture->imageData[cswap] ^ texture->imageData[cswap + 2];
		texture->imageData[cswap+2] = texture->imageData[cswap] ^ texture->imageData[cswap + 2];
		texture->imageData[cswap]   = texture->imageData[cswap] ^ texture->imageData[cswap + 2];
	}
